...
public:
			...
			if (IS_JOY_AXIS(val)) {
				...
				switch (GET_JOY_AXIS(val)) {
					...
					case AXIS_Y:
						...
						break;
					case AXIS_Z:
						sprintf(buf, "Joy%d Axis %cZ %i", GET_JOY_NUM(val),
							((isneg) ? '-' : '+'), (signed short) (val & 0xFFFF));
						break;

					case AXIS_RX:
						sprintf(buf, "Joy%d rAxis %cX %i", GET_JOY_NUM(val),
							((isneg) ? '-' : '+'), (signed short) (val & 0xFFFF));
						break;
					case AXIS_RY:
						sprintf(buf, "Joy%d rAxis %cY %i", GET_JOY_NUM(val),
							((isneg) ? '-' : '+'), (signed short) (val & 0xFFFF));
						break;
					case AXIS_RZ:
						sprintf(buf, "Joy%d rAxis %cZ %i", GET_JOY_NUM(val),
							((isneg) ? '-' : '+'), (signed short) (val & 0xFFFF));
						break;
	...
	static inline bool isAnalogState(int st) {
		return (st >= ANALOG_1 && st <= ANALOG_4);
	}
