...
int TTX_InputManager::Poll()
{
	...
	for (int i = P1_START; i < __INPUT_MAX__; i++) {
		...
		if (IS_JOY_OBJECT(val)) {
			...
			// Analógico apenas com controles :P
			if (isAnalogState(i) && IS_JOY_AXIS(val)) {
				switch (GET_JOY_AXIS(val))
				{
				case AXIS_X:
					stateTable[i] = js[joyNum].lX;
					break;

				case AXIS_Y:
					stateTable[i] = js[joyNum].lY;
					break;

				case AXIS_Z:
					stateTable[i] = js[joyNum].lZ;
					break;

				case AXIS_RX:
					stateTable[i] = js[joyNum].lRx;
					break;

				case AXIS_RY:
					stateTable[i] = js[joyNum].lRy;
					break;

				case AXIS_RZ:
					stateTable[i] = js[joyNum].lRz;
					break;
				}
...
int TTX_InputManager::Find()
{
	...
	for (list<LPDIRECTINPUTDEVICE8>::iterator pdev = lpJoysticks.begin(); pdev != lpJoysticks.end(); pdev++) {
		...
		if ((*pdev)->GetDeviceState(sizeof( DIJOYSTATE2 ), &js) == DI_OK) {
			...
			if (js.lX > DEADZONE)
				return JOYV(distance(lpJoysticks.begin(), pdev), JOYAXIS(AXIS_X, 0, js.lX));

			if (js.lX < -DEADZONE)
				return JOYV(distance(lpJoysticks.begin(), pdev), JOYAXIS(AXIS_X, 1, js.lX));

			if (js.lY > DEADZONE)
				return JOYV(distance(lpJoysticks.begin(), pdev), JOYAXIS(AXIS_Y, 0, js.lY));
			if (js.lY < -DEADZONE)
				return JOYV(distance(lpJoysticks.begin(), pdev), JOYAXIS(AXIS_Y, 1, js.lY));

			if (js.lZ > DEADZONE)
				return JOYV(distance(lpJoysticks.begin(), pdev), JOYAXIS(AXIS_Z, 0, js.lZ));
			if (js.lZ < -DEADZONE)
				return JOYV(distance(lpJoysticks.begin(), pdev), JOYAXIS(AXIS_Z, 1, js.lZ));

			if (js.lRx > DEADZONE)
				return JOYV(distance(lpJoysticks.begin(), pdev), JOYAXIS(AXIS_RX, 0, js.lRx));

			if (js.lRx < -DEADZONE)
				return JOYV(distance(lpJoysticks.begin(), pdev), JOYAXIS(AXIS_RX, 1, js.lRx));

			if (js.lRy > DEADZONE)
				return JOYV(distance(lpJoysticks.begin(), pdev), JOYAXIS(AXIS_RY, 0, js.lRy));
			if (js.lRy < -DEADZONE)
				return JOYV(distance(lpJoysticks.begin(), pdev), JOYAXIS(AXIS_RY, 1, js.lRy));

			if (js.lRz > DEADZONE)
				return JOYV(distance(lpJoysticks.begin(), pdev), JOYAXIS(AXIS_RZ, 0, js.lRz));
			if (js.lRz < -DEADZONE)
				return JOYV(distance(lpJoysticks.begin(), pdev), JOYAXIS(AXIS_RZ, 1, js.lRz));
...
void TTX_InputManager::Setup()
{
	...
	wndcls.lpszClassName = "dinput_poll";
